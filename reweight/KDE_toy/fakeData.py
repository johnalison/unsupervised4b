...
def main(seed = 5, sigma = 15, SR_id = None, loop = True, plot = False):
    ...
    # np.save('data3b_toy_seed_'+str(seed)+'.npy', data3b)
    # np.save('data4b_toy_seed_'+str(seed)+'.npy', data4b)
    ...
    binLoopStart, binLoopEnd = 0, len(m4jBinEdges);
    if SR_id is not None:
        binLoopStart, binLoopEnd = int(SR_id), int(SR_id) + 1
    
    for binNo in range(binLoopStart, binLoopEnd):
        if binNo == 0:
        ...
        num = getKDEwJMC(leadStM_SR3b, sublStM_SR3b, leadStM_SB4b, sublStM_SB4b, ttWeight = wDtoM_SB4b, nSJDat = None, nSJtt = None, sigma = sigma, loop = loop)
        den = getKDEwJMC(leadStM_SR3b, sublStM_SR3b, leadStM_SB3b, sublStM_SB3b, ttWeight = wDtoM_SB3b, nSJDat = None, nSJtt = None, sigma = sigma, loop = loop)
        outputfile = 'test_w3to4_toy_seed_'+str(seed)+'_sigma_'+str(sigma)
...
if __name__ == '__main__':
    ...
    parser.add_argument('-sig', '--sigma', default=15)
    parser.add_argument('-p', '--plot', default=False)
    parser.add_argument('-i', '--iSR', default=None)
    parser.add_argument('-l', '--loop', default=True)
    args = parser.parse_args()
    main(int(args.seed), int(args.sigma), SR_id = args.iSR , loop = bool(args.loop), plot = args.plot)
