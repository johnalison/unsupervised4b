...
parser.add_argument('-p', '--pathDtoM', default="root://cmseos.fnal.gov//store/user/smurthy/condor/unsupervised4b/randPair/wDtoM/")
...
outputfile =  "data"+year+"_picoAOD_3b_wJCM_newSBDef_w3to4.root"
...
if args.sigIn != '0':
    ...
    outputfile =  "data"+year+"_picoAOD_3b_wJCM_newSBDef_ZH4b_"+str(args.sigIn)+"percentGauss"+str(args.mean)+"-"+str(args.std)+"_w3to4.root"
...
for binNo in range(len(m4jBinEdges)):
    ...
    sigma = 5
    if mcPseudoTagWeight == "mcPseudoTagWeight":
        num = getKDEwJMC(leadStM_SR3b, sublStM_SR3b, leadStM_SB4b, sublStM_SB4b, ttWeight = wDtoM_SB4b, sigma = sigma)
        den = getKDEwJMC(leadStM_SR3b, sublStM_SR3b, leadStM_SB3b, sublStM_SB3b, ttWeight = wDtoM_SB3b, sigma = sigma)
    elif mcPseudoTagWeight == 'weight':
